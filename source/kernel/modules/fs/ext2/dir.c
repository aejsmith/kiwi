...
int ext2_dir_insert(ext2_inode_t *dir, ext2_inode_t *inode, const char *name) {
	...
	/* Work out how many blocks to read. */
	count = ROUND_UP(le32_to_cpu(dir->disk.i_size), dir->mount->blk_size) / dir->mount->blk_size;

	/* Allocate a buffer to read the data into. Don't use MM_SLEEP as it
	 * could be rather large. */
	if(!(buf = kmalloc(count * dir->mount->blk_size, 0))) {
		return -ERR_NO_MEMORY;
	}
	...
	while(current < le32_to_cpu(dir->disk.i_size)) {
		...
		if((ret = ext2_inode_write(dir, buf, 0, count, false)) != count) {
			dprintf("ext2: could not write all directory data for inode %p(%" PRId32 ") (%d)\n",
				dir, dir->num, ret);
			kfree(buf);
			return (ret < 0) ? ret : -ERR_DEVICE_ERROR;
		}
	...
	for(i = 0; i < EXT2_NDIR_BLOCKS; i++) {
		...
		dirent = kcalloc(1, dir->mount->blk_size, MM_SLEEP);
		dirent->inode = cpu_to_le32(inode->num);
		dirent->rec_len = cpu_to_le16(dir->mount->blk_size);
		dirent->name_len = name_len;
		dirent->file_type = ext2_type_to_dirent(le16_to_cpu(inode->disk.i_mode));
		...
		if((ret = ext2_block_write(dir->mount, dirent, raw, false)) != 1) {
			dprintf("ext2: could not write new block for inode %p(%" PRId32 ") (%d)\n",
			        dir, dir->num, ret);
			kfree(dirent);
			kfree(buf);
			return (ret < 0) ? ret : -ERR_DEVICE_ERROR;
		...
		dir->disk.i_size = cpu_to_le32(le32_to_cpu(dir->disk.i_size) + dir->mount->blk_size);
		I_BLOCKS_INC(dir);
 ...
 * @param dir		Directory to check (read-/write-locked).
 * @return		True if empty, false if not (or on error). */
bool ext2_dir_empty(ext2_inode_t *dir) {
	...
	/* Check if the directory is empty apart from the . and .. entries. */
	while(current < le32_to_cpu(dir->disk.i_size)) {
		dirent = (ext2_dirent_t *)(buf + current);
		current += le16_to_cpu(dirent->rec_len);
		...
		if(le32_to_cpu(dirent->inode) == 0) {
			continue;
		} else if(dirent->name_len == 1 && strncmp(dirent->name, ".", 1) == 0) {
			continue;
		} else if(dirent->name_len == 2 && strncmp(dirent->name, "..", 2) == 0) {
			continue;
		}
